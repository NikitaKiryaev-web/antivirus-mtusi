...
void BaseEditor::read(const std::u16string& path)
{
	...
	if (header != std::u16string(u"Kiryaev"))
		return;
...
void BaseEditor::write(const std::u16string& path)
{
	...
	std::u16string header(u"Kiryaev");
...
{
	QFileDialog* dialog = new QFileDialog(nullptr, "OpenFileDialog", "./", "*.kir");
	dialog->show();
	if (dialog->exec())
		write(dialog->selectedFiles()[0].toStdU16String());
}
...
void BaseEditor::on_readButton_clicked()
{
	...
	dialog->show();
	if (dialog->exec())
		read(dialog->selectedFiles()[0].toStdU16String());
}
